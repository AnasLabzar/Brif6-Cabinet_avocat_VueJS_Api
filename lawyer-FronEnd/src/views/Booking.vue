  ...
  <div class="Booking">
    <div class="formContainer">
      ...
      <form action="" @submit.prevent="insert">
        ...
        <div class="formControl">
          <select @focus="toggleDateAndTimeClass " v-model="TimeSlots" @change="checkTime">
...
export default {
  ...
  methods: {
    ...
    async checkTime() {
      let resp = await fetch("http://localhost/Back-End/rendezVous/checkTime", {
        method: "POST",
        body: JSON.stringify({
          Date: this.date,
        }),
      ...
      let availableTimeSlots = new Array();
      data.forEach((result) => {
        let timeHM = result.time.split(":");
        let timeS = timeHM.pop();
        let timeStr = timeHM.join(":");
        availableTimes.push(timeStr);
      });

      availableTimeSlots = this.time.filter((t) => !availableTimes.includes(t));
      this.time = availableTimeSlots;
